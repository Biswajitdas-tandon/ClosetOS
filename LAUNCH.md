...
- [ ] **Google sign-in** — hidden by default (`NEXT_PUBLIC_GOOGLE_AUTH_ENABLED` unset). To enable: create a Google Cloud OAuth client (Web application; authorised redirect URI `https://ncowtwpxvefhbljwfsxo.supabase.co/auth/v1/callback`), paste client ID/secret in Dashboard → Auth → Providers → Google, then set `NEXT_PUBLIC_GOOGLE_AUTH_ENABLED=true` in Vercel Production and redeploy.
